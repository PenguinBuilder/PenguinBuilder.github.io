...
import{t as I}from"./theme-selector.js";import{$ as u}from"./jsquery.js";const q="modulepreload",E=function(n,e){return new URL(n,e).href},k={},s=function(e,o,i){let r=Promise.resolve();if(o&&o.length>0){let A=function(t){return Promise.all(t.map(h=>Promise.resolve(h).then(g=>({status:"fulfilled",value:g}),g=>({status:"rejected",reason:g}))))};const a=document.getElementsByTagName("link"),l=document.querySelector("meta[property=csp-nonce]"),w=l?.nonce||l?.getAttribute("nonce");r=A(o.map(t=>{if(t=E(t,i),t in k)return;k[t]=!0;const h=t.endsWith(".css"),g=h?'[rel="stylesheet"]':"";if(i)for(let d=a.length-1;d>=0;d--){const y=a[d];if(y.href===t&&(!h||y.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${t}"]${g}`))return;const c=document.createElement("link");if(c.rel=h?"stylesheet":q,h||(c.as="script"),c.crossOrigin="",c.href=t,w&&c.setAttribute("nonce",w),document.head.appendChild(c),h)return new Promise((d,y)=>{c.addEventListener("load",d),c.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${t}`)))})}))}function f(a){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=a,window.dispatchEvent(l),!l.defaultPrevented)throw a}return r.then(a=>{for(const l of a||[])l.status==="rejected"&&f(l.reason);return e().catch(f)})},O=`<section>
<h1>[#6C9ACE] Block</h1>
<hr>
<section>
<h2>Create Block</h2>
...
`;I();const x=await s(()=>import("./render.js"),__vite__mapDeps([0,1,2]),import.meta.url);x.onDone(()=>{setTimeout(()=>{const n=location.hash.slice(1);n?j(n):p.children.length>0&&j(p.children[0].getProp("for"))},500)});const b={};await Promise.all(Object.entries(Object.assign({"./images/JSON_from.json":()=>s(()=>import("./JSON_from.js"),[],import.meta.url),"./images/call_hat.json":()=>s(()=>import("./call_hat.js"),[],import.meta.url),"./images/create_block.json":()=>s(()=>import("./create_block.js"),[],import.meta.url),"./images/create_dynamic_menu.json":()=>s(()=>import("./create_dynamic_menu.js"),[],import.meta.url),"./images/create_hat.json":()=>s(()=>import("./create_hat.js"),[],import.meta.url),"./images/create_input_from_menu.json":()=>s(()=>import("./create_input_from_menu.js"),[],import.meta.url),"./images/create_input_menu.json":()=>s(()=>import("./create_input_menu.js"),[],import.meta.url),"./images/create_menu.json":()=>s(()=>import("./create_menu.js"),[],import.meta.url),"./images/order_category.json":()=>s(()=>import("./order_category.js"),[],import.meta.url),"./images/switch.json":()=>s(()=>import("./switch.js"),[],import.meta.url)})).map(async([n,e])=>b[n]=(await e()).default));const m=u("#content");m.html(O);m.all("img").forEach(async n=>{const e=n.getProp("src");if(e.startsWith("./images/")){if(e.endsWith(".json")){x.registerElt(n,await(await fetch(b[e])).json());return}n.props({src:b[e]})}});const p=u("#toc"),D=u.all("#content>section");function P(n,e,o){return u.create("div").child([u.create("div").css({"border-radius":"100%",background:n,width:"20px",height:"20px"}).class("circ"),u.create("div").css({fontSize:"16px",background:"#00000000"}).text(e)]).css({display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"5px",padding:"5px"}).click(()=>{j(o)}).props({for:o})}D.forEach(n=>{const e=n.$("h1").text().match(/^\[(#[0-9a-fA-F]{6})\]\s*(.+)/);if(!e)return;const o=e[1],i=e[2];n.id(i.toLowerCase().trim().replaceAll(/[^a-z0-9]/g,"-")),n.$("h1").text(i);const r=P(o,i,n.id());p.child(r)});let _=!1;function j(n){history.pushState(null,"","#"+n);const e=u("#"+n);e&&(p.all("[selected]").props({selected:null}),p.$(`[for=${n}]`).props({selected:""}),_=!0,e.elt.scrollIntoView({behavior:"smooth"}))}m.on("scrollend",()=>{_=!1});let v=0;m.on("scroll",()=>{if(_)return;const n=m.elt.scrollTop,e=m.children,o=n>v;for(const i of e){const r=i.rect(),f=r.top>=0&&r.top<window.innerHeight,a=r.bottom>0&&r.bottom<=window.innerHeight;if(o?f:a){p.all("[selected]").props({selected:null}),p.$(`[for=${i.id()}]`).props({selected:""}),history.pushState(null,"","#"+i.id());break}v=n}});
